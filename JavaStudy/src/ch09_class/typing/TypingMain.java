...
public class TypingMain {
	...
	public static void main(String[] args) {
		...
		// 단어 목록
		// 문자를 담는 리스트
		// 순서 1. 객체생성, 2.  3. 메소드 생성
		// public static 메소드는 해당 클래스를 굳이 객체로 만들지
		// 않더라도 사용이 가능하다.
		
//		예시) ArrayList <String> wordList = dict.makeWordList();
		...
		// 현재 시간 저장
		// 1970년 1월 1일을 기준으로
		// 현재까지 경과된 시간을 밀리초 단위로 리턴한다. (1초 = 1000밀리초)
		long before = System.currentTimeMillis();
		
		while(true) {
			// 1. wordList에 담긴 단어가 랜덤하게 출력
			...
			// 일치하지 않는다면 해당 단어가 삭제되지 않고 또 입력
			
			// 위 과정을 반복실행해서 (wordList.size()가 0이 될때까지)
			if(wordList.size()==0) {
				break;
			}
		}
